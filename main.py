...
def text_recognition():
    ...
    if image_file:
        ...
        # Process the results (e.g., display text, play audio)此行可以成功读出
        #return render_template('text-recognition-capture.html', text_output=text, img_path=adjusted_image_path, audio_path=audio_file, timestamp=timestamp)

        return render_template('text-recognition-upload.html', text=text, audio_path=audio_file, adjusted_image_path=adjusted_image_path, timestamp=timestamp)
